...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        B, C = x_t.shape[:2]
        assert t.shape == (B,)
        raw_output = model(self._scale_input(x_t, t), t, **model_kwargs)

        # Handle confidence map output
        confidence_map = None
        if isinstance(raw_output, tuple):
            model_output = raw_output[0]
            confidence_map = raw_output[1]  # [N, 1, h, w]
        else:
    ...
    def p_sample(self, model, x, y, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, noise_repeat=False):
        ...
        result = {"sample": sample, "pred_xstart": out["pred_xstart"], "mean":out["mean"]}
        if "confidence_map" in out:
            result["confidence_map"] = out["confidence_map"]
        return result
    ...
    def p_sample_loop_with_confidence(
        self,
        y,
        model,
        first_stage_model=None,
        ...
        """
        Like p_sample_loop but also returns the confidence map from the last
        denoising step (latent space, before VQGAN decode).

        Returns:
            (decoded_output, confidence_map_latent)
            confidence_map_latent: [N, 1, h, w] in latent space or None
        """
        final = None
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            final = sample["sample"]
            if "confidence_map" in sample:
                last_confidence = sample["confidence_map"]
        ...
        # Upsample confidence map from latent to pixel space if needed
        if last_confidence is not None and out.shape[2:] != last_confidence.shape[2:]:
            last_confidence = F.interpolate(
                last_confidence,
                size=out.shape[2:],
                mode='bilinear',
                align_corners=False,
    ...
    def training_losses(
        ...
        :param cdiff_beta: if not None, use C-Diff loss with this beta value.
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
            raw_output = model(self._scale_input(z_t, t), t, **model_kwargs)

            # Handle confidence map output from UNet
            confidence_map = None
            if isinstance(raw_output, tuple):
                model_output, confidence_map = raw_output  # [N,C,h,w], [N,1,h,w]
            else:
                model_output = raw_output
            ...
            if confidence_map is not None and cdiff_beta is not None and cdiff_beta > 0:
                # ===== C-Diff Loss (C-DiffSET paper, Eq. 3) =====
                tau = 0.5 * math.log(2 * math.pi)
                
                max_c_beta = math.exp(tau)
                
                # Ép confidence_map > 1e-6 để chống lỗi NaN khi base <= 0
                safe_confidence = th.clamp(confidence_map, min=1e-6)
                
                # Tính c_beta và KHÓA TRẦN (Clamp max) để -log(c_beta) + tau luôn >= 0
                c_beta = (safe_confidence ** cdiff_beta).clamp(max=max_c_beta)

                residual_sq = (target - model_output) ** 2  # [N, C, h, w]
                weighted_mse = c_beta * residual_sq
                
                # Vì đã clamp min/max nên c_beta rất an toàn, không cần + 1e-8
                log_c_beta = th.log(c_beta)         # [N, 1, h, w]
                reg_term = -log_c_beta + tau

                # Per-sample loss
                cdiff_loss = mean_flat(weighted_mse) + mean_flat(reg_term)
                terms["mse"] = cdiff_loss
                terms["confidence_mean"] = confidence_map.detach().mean(dim=[1,2,3])
            else:
                # Standard MSE loss
            ...
            # ===== Gradient Loss (G-Grad) =====
            # Penalises differences in spatial gradients (edges) between
            # predicted and target latent codes.  Uses Sobel-like finite
            # differences in both x and y directions.
            def _spatial_gradient(x):
                """Return (grad_x, grad_y) each [N,C,H,W]."""
                gx = x[:, :, :, :-1] - x[:, :, :, 1:]   # horizontal
                gy = x[:, :, :-1, :] - x[:, :, 1:, :]   # vertical
                return gx, gy

            pred_gx, pred_gy = _spatial_gradient(model_output)
            tgt_gx,  tgt_gy  = _spatial_gradient(target)
            grad_loss = mean_flat((pred_gx - tgt_gx) ** 2) + mean_flat((pred_gy - tgt_gy) ** 2)
            terms["grad"] = grad_loss
