...
class UNetModelSwin(nn.Module):
    ...
    def __init__(
        ...
        # Determine base LQ channels (lq_channels, or +1 if cond_mask)
        lq_chn_with_mask = (lq_channels + 1) if cond_mask else lq_channels

        if cond_lq and lq_size == image_size:
            self.feature_extractor = nn.Identity()
            base_chn = lq_chn_with_mask
        else:
            feature_extractor = []
            feature_chn = lq_chn_with_mask
            base_chn = 16
        ...
        if self.use_confidence:
            ...
            # Initialize bias so that SoftPlus(bias) ≈ 1.0 → bias = log(e - 1) ≈ 0.5413
            ...
            self.confidence_head = nn.Sequential(
                ...
                nn.SiLU(),
                confidence_conv,
                nn.Softplus(),  # ensures non-negative confidence
            )
