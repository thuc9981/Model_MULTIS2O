...
class BaseSampler:
    ...
    def build_model(self):
        ...
        ckpt = util_common.torch_load(ckpt_path, map_location=f"cuda:{self.rank}")
    ...
    def load_model(self, model, ckpt_path=None):
        state = util_common.torch_load(ckpt_path, map_location=f"cuda:{self.rank}")
...
class ResShiftSampler(BaseSampler):
    def sample_func(self, y0, noise_repeat=False, mask=False):
        ...
        # For dual-band SAR (2ch), expand VV to 3ch for VQGAN autoencoder
        is_dualband = (y0.shape[1] == 2)
        if is_dualband:
            y_diffusion = y0[:, 0:1, :, :].expand(-1, 3, -1, -1)
        else:
            y_diffusion = y0
    ...
    def sample_func_with_confidence(self, y0, noise_repeat=False, mask=False):
        '''
        Same as sample_func but also returns the confidence map.
        Input:
            y0: n x c x h x w torch tensor, low-quality image, [-1, 1]
        Output:
            sample: n x c x h x w, torch tensor, [-1, 1]
            confidence: n x 1 x H x W, torch tensor (pixel space) or None
        '''
        if noise_repeat:
            self.setup_seed()
        ...
        results, confidence = self.base_diffusion.p_sample_loop_with_confidence(
                y=y_diffusion,
                model=self.model,
                first_stage_model=self.autoencoder,
                noise=None,
        ...
        if flag_pad:
            ...
            if confidence is not None:
                confidence = confidence[:, :, :ori_h*self.sf, :ori_w*self.sf]

        return results.clamp_(-1.0, 1.0), confidence
