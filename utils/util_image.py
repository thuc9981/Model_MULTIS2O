...
def batch_PSNR(img_pred, img_gt, ycbcr=False, border=0):
    """
    Hàm tính PSNR chuẩn khoa học, chống lỗi Float32.
    """
    # Ép về Numpy array
    if hasattr(img_pred, 'detach'):
        pred = img_pred.detach().cpu().numpy()
        gt = img_gt.detach().cpu().numpy()
    else:
        pred = np.array(img_pred)
        gt = np.array(img_gt)
        
    # Cắt xén các giá trị dị thường (Outliers) do AI sinh ra
    # Ép chặt dải giá trị về [0.0, 1.0] để tính PSNR chuẩn xác
    ...
    for i in range(batch_size):
        # skimage sẽ tự tính PSNR với data_range=1.0 cực chuẩn
        psnr_val = compare_psnr(gt[i], pred[i], data_range=1.0)
        total_psnr += psnr_val
...
def bgr2rgb(im):
    ...
    if im.ndim == 2:
        return im
    if im.ndim == 3 and im.shape[2] in (1, 2):
        # OpenCV does not support 1/2-channel BGR<->RGB. Expand to 3 channels.
        if im.shape[2] == 1:
            im = np.repeat(im, 3, axis=2)
        else:
            pad = np.zeros_like(im[:, :, :1])
...
def rgb2bgr(im):
    ...
    if im.ndim == 2:
        return im
    if im.ndim == 3 and im.shape[2] in (1, 2):
        # OpenCV does not support 1/2-channel RGB<->BGR. Expand to 3 channels.
        if im.shape[2] == 1:
            im = np.repeat(im, 3, axis=2)
        else:
            pad = np.zeros_like(im[:, :, :1])
...
class ImageSpliterTh:
    def __init__(self, im, pch_size, stride, sf=1, extra_bs=1, weight_type=None):
        '''
        Input:
            ...
            weight_type: None/'uniform' for equal weights, 'hann' for smooth blending
        '''
        assert stride <= pch_size
        self.stride = stride
        self.pch_size = pch_size
        ...
        # Precompute blending weight (optional)
        self.weight = None
        if self.weight_type is not None and str(self.weight_type).lower() != 'uniform':
            if str(self.weight_type).lower() == 'hann':
                win = torch.hann_window(self.pch_size, periodic=False, device=im.device, dtype=im.dtype)
                weight_2d = win[:, None] * win[None, :]
                self.weight = weight_2d[None, None, :, :]
            else:
                raise ValueError(f"Unsupported weight_type: {self.weight_type}. Use None/'uniform' or 'hann'.")
    ...
    def update(self, pch_res, index_infos):
        ...
        for ii, (h_start, h_end, w_start, w_end) in enumerate(index_infos):
            ...
            if self.weight is None:
                ...
                self.im_res[:, :, h_start:h_end, w_start:w_end] += current_pch * self.weight
